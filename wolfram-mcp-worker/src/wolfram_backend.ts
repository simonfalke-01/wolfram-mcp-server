...
export interface ExecuteWolframRequest {
  code: string;
  timeout?: number;
}

export interface WolframAlphaRequest {
  query: string;
  timeout?: number;
  format?: string;
}
...
export class WolframLanguageClient {
   ...
   */
  async executeWolfram(
    request: ExecuteWolframRequest,
   ...
   */
  async queryWolframAlpha(
    request: WolframAlphaRequest,
  ): Promise<WolframResponse> {
    const response = await this.makeRequest("POST", "/wolfram-alpha", request);
    return response as WolframResponse;
  }
  ...
  private async makeRequest(
    ...
    try {
      ...
      try {
        ...
        throw new Error(
          `Invalid JSON response from Wolfram Language Server: ${responseText}`,
        );
      }
    } catch (error) {
      if (error instanceof Error) {
        if (error.name === "AbortError" || error.name === "TimeoutError") {
          throw new Error(
            `Wolfram Language Server request timed out after ${this.timeout}ms`,
          );
        }
        throw error;
      }
      throw new Error(
        `Unknown error communicating with Wolfram Language Server: ${String(error)}`,
      );
    }
  }
